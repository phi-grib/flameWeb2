...
export class PredictionComponent implements OnInit {
  ...
  plotComboQ = {
    ...
    layout: {
      ...
      hovermode: "x",
      hoverlabel: {
        font: { family: "Barlow Semi Condensed, sans-serif", size: 14 },
      },
      xaxis: {
        zeroline: false,
        tickfont: { family: "Barlow Semi Condensed, sans-serif", size: 14 },
    ...
    if ("PC1proj" in this.prediction.result) {
      ...
      if ("PCDMODX" in this.prediction.result) {
        ...
        this.plotScores.data[1].marker.color =
          this.prediction.result["PCDMODX"];
  ...
  updatePlotCombo() {
    ...
    this.isMajority =
      this.prediction.modelBuildInfo["model"] ==
        "combination:majority voting" ||
    ...
    if (!this.isMajority) {
      ...
      for (let i = 0; i < this.prediction.result.var_nam.length; i++) {
        ...
        if (this.isQuantitative) {
          ...
          this.plotComboQ.data[1].x[i] =
            this.prediction.result.values[this.compound.molidx];
        }
      }
      var drawCI = false;
      if (this.prediction.result["ensemble_ci"]) {
        ...
        // support for legacy models where we used ensemble_confidence
        if (this.prediction.result["ensemble_confidence"]) {
          drawCI = true;
          var cilist =
            this.prediction.result.ensemble_confidence[this.compound.molidx];
        }
      }
      if (drawCI) {
        for (let i = 0; i < this.prediction.result.var_nam.length; i++) {
        ...
        if (this.isQuantitative && this.prediction.result["upper_limit"]) {
          for (let i = 0; i < this.prediction.result.var_nam.length; i++) {
            ...
            this.plotComboQ.data[2].x[i] =
              this.prediction.result.upper_limit[this.compound.molidx];
          }
          ...
          for (let i = this.prediction.result.var_nam.length - 1; i > -1; i--) {
            ...
            this.plotComboQ.data[2].x[j] =
              this.prediction.result.lower_limit[this.compound.molidx];
            j++;
          }
        }
      }
    ...
    // TODO: show ensemble prediction
      ...
      if (this.prediction.result["ensemble_ci"]) {
        drawCI = true;
        var class_list =
          this.prediction.result.ensemble_ci[this.compound.molidx];
      } else {
        // support for legacy models where we used ensemble_confidence
        if (this.prediction.result["ensemble_confidence"]) {
          drawCI = true;
