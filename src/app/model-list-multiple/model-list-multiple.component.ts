...
declare var $: any;
@Component({
  selector: 'app-model-list-multiple',
  templateUrl: './model-list-multiple.component.html',
  styleUrls: ['./model-list-multiple.component.css']
})
export class ModelListMultipleComponent implements OnInit {
  models: Array<any>;
  ...
  loadCollection(collect: Object) {
    ...
    $('#dataTableModels').DataTable().rows().every(function (idx, tableLoop, rowLoop) {
      var node = this.node()
      let checkbox = node.childNodes[0].childNodes[0] as HTMLInputElement
